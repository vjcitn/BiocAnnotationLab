...
packages in the BioconductorAnnotationPipeline system.
    - We will focus on another approach to Gene Ontology (and indeed all ontologies at OBOFoundry)
...
in footprint.  See, for example, [experimental lkparq package](https://vjcitn.github.io/lkparq/articles/lkparq.html).
...
# Low level approach to GO interface

For strategizing how we can connect GO to
gene annotations in the GAF format produced
by Gene Ontology Consortium, we have included some 
basic code for working with Semantic SQL representation of GO
in this exploratory package.  Some of it may be migrated
to ontoProc2 after evaluation of strategies.

```{r lk,message=FALSE}
library(BiocAnnotationLab)
con = make_go_con()
yy = go_terms(con) 
yy |> dplyr::filter(label %like% "%apopto%")
```
